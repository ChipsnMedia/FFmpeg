                                               ...
                                               char *data, size_t *data_len)
{
    ...
    if(fh->delta_q_y_dc){
        put_bits(&pbc, 1, 1);
        put_sbits(&pbc, 7, fh->delta_q_y_dc);
    }
    else
        put_bits(&pbc, 1, 0);
    
    if(sh->color_config.separate_uv_delta_q)
        put_bits(&pbc, 1, fh->diff_uv_delta);
        
    if(fh->delta_q_u_dc){
        put_bits(&pbc, 1, 1);
        put_sbits(&pbc, 7, fh->delta_q_u_dc);
    }
    else
        put_bits(&pbc, 1, 0);
    ...
    if(fh->delta_q_u_ac){
        put_bits(&pbc, 1, 1);
        put_sbits(&pbc, 7, fh->delta_q_u_ac);
    }
    else
        put_bits(&pbc, 1, 0);
    ...
    if(fh->using_qmatrix){
        put_bits(&pbc, 4, fh->qm_y);
        put_bits(&pbc, 4, fh->qm_u);
        if(sh->color_config.separate_uv_delta_q)
            put_bits(&pbc, 4, fh->qm_v);
    }
    ...
    if (!(coded_lossless || fh->allow_intrabc)) {
        ...
        if (fh->loop_filter_delta_enabled)
            put_bits(&pbc, 1, fh->loop_filter_delta_update);
    }
...
//+tony the setting for wave6 encoder
    sh->seq_profile = 0;
    sh->still_picture = 0;
    sh->reduced_still_picture_header = 0;
    sh->decoder_model_info_present_flag = 0;
    sh->initial_display_delay_present_flag = 0;
    sh->frame_id_numbers_present_flag = 0;
    sh->use_128x128_superblock = 0;
    sh->enable_filter_intra = 0;
    ...
    sh->seq_choose_screen_content_tools = 0;
    sh->seq_force_screen_content_tools = 0;
    sh->order_hint_bits_minus_1 = 6;
    sh->enable_superres = 0;
    sh->film_grain_params_present = 0;
    
    sh->timing_info.equal_picture_interval = 1;
    sh->timing_info.num_ticks_per_picture_minus_1 = 0;
    
    sh->color_config.mono_chrome = 0;
    sh->color_config.color_description_present_flag = 0;
    sh->color_config.color_range = 0;
                                                ...
                                                VAAPIEncodePicture *pic)
{
    ...
    AV1RawOBU                    *sh_obu = &priv->sh;
    AV1RawFrameHeader                *fh = &fh_obu->obu.frame.header;
    AV1RawSequenceHeader             *sh = &sh_obu->obu.frame.header;
    VAEncPictureParameterBufferAV1 *vpic = pic->codec_picture_params;
    CodedBitstreamFragment          *obu = &priv->current_obu;
...
const AVCodec ff_av1_vaapi_encoder = {
    .name           = "av1_vaapi",
